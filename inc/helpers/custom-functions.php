<?php
/**
 * Rt Dev Site Alert custom functions.
 *
 ...
 */
function rt_dev_site_alert_get_cache_key( $unique = '' ) {

	$cache_key = 'rt_dev_site_alert_cache_';
 ...
 */
function rt_dev_site_alert_get_cached_posts( $args ) {
	...
	$cache_key = rt_dev_site_alert_get_cache_key( $args );

	$cache  = new \Rt_Dev_Site_Alert\Inc\Cache( $cache_key );
 ...
 */
function rt_dev_site_alert_get_template_content( $slug, $vars = [] ) {
 ...
 */
function rt_dev_site_alert_template( $template, $variables = [], $echo = false ) {

	$template_file = sprintf( '%1$s/templates/%2$s.php', RT_DEV_SITE_ALERT_PATH, $template );
 ...
 */
function rt_dev_site_alert_get_data( $slug, $default = [] ) {

	$data_file = sprintf( RT_DEV_SITE_ALERT_PATH . '/inc/data/%s.php', $slug );
 ...
 */
function rt_dev_site_alert_is_production() {
 ...
 */
function rt_dev_site_alert_is_mobile( $kind = 'any', $return_matched_agent = false ) {
